...
class MDPNode:
    def __init__(self, rest: str, hw: str, time: str, actions: list, reward: list, transition: list, terminal = False):
...
def create_tree():
    root = MDPNode("R", "U", "8p", ["P", "R", "S"], [2, 0, -1], [1.0, 1.0, 1.0])
    tree = MDPGraph(root)
    child1 = MDPNode("T", "U", "10p", ["P", "R"], [2, 0], [1.0, 1.0])
    child2 = MDPNode("R", "U", "10p", ["P", "R", "S"], [0, [2, 2], -1], [1.0, [.5, .5], 1.0])
    child3 = MDPNode("R", "D", "10p", ["P", "R"], [[2, 2], 0], [[.5, .5], 1.0])
    tree.insert_children(tree.root, [child1, child2, child3])
    curr = tree.root.children[0]
    child4 = MDPNode("R", "U", "10a", ["P", "R", "S"], [0, 0, 0], [1.0, 1.0, 1.0])
    child5 = MDPNode("R", "U", "8a", ["P", "R", "S"], [2, 0, -1], [1.0, 1.0, 1.0])
    tree.insert_children(curr, [child4, child5])
    curr = tree.root.children[1]
    child6 = MDPNode("R", "D", "8a", ["P", "R"], [2, 0], [1.0, 1.0])
    tree.insert_children(curr, [child5, [child5, child4], child6])
    curr = tree.root.children[2]
    child7 = MDPNode("R", "D", "10a", ["P", "R", "S"], [4, 4, 4], [1.0, 1.0, 1.0])
    tree.insert_children(curr, [child6, [child6, child7]])
    curr = tree.root.children[1]
    curr = curr.children[0]
    child8 = MDPNode("T", "U", "10a", ["P", "R", "S"], [-1, -1, -1], [1.0, 1.0, 1.0])
    tree.insert_children(curr, [child8, child4, child7])
    curr = tree.root.children[2]
    curr = curr.children[0]
    child9 = MDPNode("T", "D", "10a", ["P", "R", "S"], [3, 3, 3], [1.0, 1.0, 1.0])
    tree.insert_children(curr, [child7, child9])
    class_node = MDPNode("", "", "11a", [], [], [], True)
    curr = curr.children[0]
